    ...
    a = SphericalPolygon(
        ArcString(
            MultiSphericalPoint.from_lonlat(
                np.array([[20.0, 5.0], [25.0, 5.0], [25.0, 10.0], [20.0, 10.0]]),
            )
        ),
    )
...
def test_intersects_simple():
    ...
    poly1 = SphericalPolygon(
        MultiSphericalPoint.from_lonlat(np.stack((lon1, lat1), axis=1))
    )

    ra2 = np.array([-5, 15, 15, -5, -5], dtype=float)
    dec2 = np.array([20, 20, -10, -10, 20], dtype=float)
    poly2 = SphericalPolygon(
        MultiSphericalPoint.from_lonlat(np.stack((ra2, dec2), axis=1))
    )
...
def test_intersects_poly_fully_contained():
    ...
    poly2 = SphericalPolygon(
        MultiSphericalPoint.from_lonlat(np.stack((lon2, lat2), axis=1))
    )
...
def test_self_intersection():
    ...
    s1 = SphericalPolygon(
        MultiSphericalPoint.from_lonlat(np.array(ra1), np.array(dec1))
    )
    s2 = SphericalPolygon(
        MultiSphericalPoint.from_lonlat(np.array(ra2), np.array(dec2))
    )
    s12 = s2.union(s1)
    # asserts self-intersection is same as original
    s12int = s12.intersection(s12)
    assert_allclose(s12int.area(), s12.area(), rtol=1e-6)
...
def test_intersection_crash():
    ...
    testcenter = np.array([0.38681003, 0.07318841, 0.91925049])
    ...
    polycenter = np.array([0.16877166, 0.19684143, 0.96579997])

    testFoV = SphericalPolygon(testpoints, inside=testcenter)
    poly = SphericalPolygon(polypoints, inside=polycenter)

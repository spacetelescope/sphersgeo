    ...
    def __call__(self, func):
        ...
        def run(*args, **kwargs):
            ...
            for i, permutation in enumerate(
                itertools.islice(itertools.permutations(polys), None, None, step_size)
            ):
                ...
                if GRAPH_MODE:
                    fig = plt.figure()
                    m = Basemap(
                        projection=self._proj, lon_0=self._lon_0, lat_0=self._lat_0
                    )
                    m.drawmapboundary(fill_color="white")
                    m.drawparallels(np.arange(-90.0, 90.0, 20.0))
                    m.drawmeridians(np.arange(0.0, 420.0, 20.0))
            ...
            lengths = np.array(
                [sum(len(x._points) for x in y.iter_polygons_flat()) for y in unions]
            )
...
def test_inside_point():
    ...
    c = np.array([1.00000000e00, 0.00000000e00, 6.12323400e-17])
